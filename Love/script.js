document.addEventListener("DOMContentLoaded", () => {
  ...
  const giantFlower = document.getElementById("giantFlower");
  ...
  function triggerFlowerAnimation() {
    if (!giantFlower) return;

    giantFlower.classList.remove("visible", "petals-off");
    void giantFlower.offsetWidth;

    giantFlower.classList.add("visible");

    setTimeout(() => {
      giantFlower.classList.add("petals-off");
    }, 800);

    const totalDuration = 800 + 2600 + 700;
    setTimeout(() => {
      giantFlower.classList.remove("visible", "petals-off");
    }, totalDuration);
  }
  ...
  if (noBtn) {
    ...
    noBtn.addEventListener("animationend", (event) => {
      if (event.animationName === "no-pop") {
        noBtn.classList.remove("no-pop");
        randomTeleportNo();
      }
    });
  }
